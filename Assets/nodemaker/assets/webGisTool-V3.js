...
$(document).ready(function () {
    ...
    console.log('asd')
    // $("#filterdata").append( "<select name='Kelompok' id='Kelompok' class='form-control'><option value='MANGGA'>MANGGA</option><option value='PEPAYA'>PEPAYA</option></select> <button class='btn btn-primary' id='bt_Search'>Search</button>" );
});
function base_url() {
...
function initMap() {
    ...
    var def_url = base_url();
    var mapOptions = loadMapStyle(def_url+'/Assets/nodemaker/ajax/mapDay.json');
    // console.log(def_url)
    map = new google.maps.Map(document.getElementById('map-canvas'),
            {
                // -7.389940899999999,110.9148985
                zoom: 10,
                center: {lat: -7.389940899999999, lng: 110.9148985},
    ...
    var locations = getVertexFromDatabase('<?php echo base_url() ?>ajax/vertex.php',$('#Kelompok').val());
    ...
    for (i = 0; i < locations.length; i++) {
        ...
        if (locations[i]["anggota"] === "C1") {
            icon_marker = base_url() + 'Assets/nodemaker/assets/marker/asal.png';
            ...
            icon_marker =base_url() + 'Assets/nodemaker/assets/marker/tujuan.png';
            max_currentVertex = Number(locations[i][0]) + 1;
            marker = new google.maps.Marker({
                position: new google.maps.LatLng(locations[i]["lat"], locations[i]["lng"]),
                map: map,
            ...
            icon_marker = base_url() +'Assets/nodemaker/assets/marker/node.png';
            max_currentVertex = Number(locations[i][0]) + 1;
            marker = new google.maps.Marker({
                position: new google.maps.LatLng(locations[i]["lat"], locations[i]["lng"]),
                map: map,
        ...
        google.maps.event.addListener(marker, 'click', (function (marker, i) {
            return function () {
                var contents = "<b>Cluster Anggota: </b>" + locations[i]["anggota"] + "<br>" +
                        "<b>Nama Kecamatan: </b>" + locations[i]["nama"] + "<br>" +
                        "<b>Alamat: </b>" + locations[i]["Alamat"] + "<br>"+
                        "<b>Keterangan: </b>" + locations[i]["Keterangan"] + "<br>"+
                        "<b>Data Tahun: </b>" + locations[i]["Tahun"] + "<br>"
...
function getVertexFromDatabase(url,filter) {
    console.log(filter)
    return JSON.parse($.ajax({
        type: 'POST',
        url: base_url() + 'C_Proses/getHasil',
        data: {'kelompok':'','NamaBuah':filter},
        dataType: 'json',
        global: false,
        async: false,
